...
import { CheckCircle, CreditCard, Package, MapPin, ClipboardList } from 'lucide-react'
...
export default function CheckoutPage() {
  ...
  if (!cartLoading && (!cart || cart.items.length === 0)) {
    return (
      <div className="max-w-xl mx-auto px-4 sm:px-6 py-20 text-center space-y-4">
        ...
        <Link href="/shop" className="btn-primary mx-auto">Browse shop</Link>
      </div>
    )
  }
      ...
      <div className="max-w-lg mx-auto px-4 sm:px-6 py-20 text-center space-y-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-[var(--muted)]">
            Order <span className="text-violet font-mono font-semibold">{placedOrderNumber}</span> is confirmed and on its way.
          </p>
        </div>
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <Link
            href={`/account/orders/${placedOrderNumber}`}
            className="btn-primary"
          >
            Track order
          </Link>
          <Link href="/shop" className="glass rounded-xl px-6 py-2.5 text-sm font-medium border border-[var(--border)] hover:border-violet/40 transition-colors text-center">
            Continue shopping
          </Link>
        </div>
      </div>
  ...
  const paymentValid =
    cardDigits.length >= 13 && !!payment.exp.match(/^\d{2}\/\d{2}$/) &&
    payment.cvc.length >= 3 && !!payment.name
          ...
          {step === 1 && (
            <div className="glass rounded-3xl p-6 space-y-4">
              ...
              <div className="flex justify-between">
                <button type="button" onClick={() => setStep(0)} className="text-sm text-[var(--muted)] hover:text-[var(--text)]">
                  ← Back
                </button>
          ...
          {step === 2 && (
            <div className="glass rounded-3xl p-6 space-y-4">
              ...
              <p className="text-xs text-[var(--muted)] bg-violet/5 border border-violet/20 rounded-xl px-3 py-2">
                This is a simulated checkout. Use any valid-format card number. Card ending <strong>0002</strong> simulates a decline.
              </p>
              <Input
              ...
              <div className="grid grid-cols-2 gap-4">
                <Input
                  label="Expiry (MM/YY)"
                  value={payment.exp}
                  onChange={(e) => setPayment((p) => ({ ...p, exp: formatExp(e.target.value) }))}
                  placeholder="MM/YY"
                  autoComplete="cc-exp"
                  inputMode="numeric"
                  maxLength={5}
              ...
              <div className="flex justify-between">
                <button type="button" onClick={() => setStep(1)} className="text-sm text-[var(--muted)] hover:text-[var(--text)]">
                  ← Back
                </button>
                <button
                  type="button"
            ...
            <div className="glass rounded-3xl p-6 space-y-6">
              ...
              <div className="space-y-1 text-sm">
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={() => setStep(0)} className="text-xs text-violet hover:underline">Edit</button>
                </div>
                <p className="text-[var(--muted)]">{address.fullName}</p>
                <p className="text-[var(--muted)]">{address.line1}{address.line2 ? `, ${address.line2}` : ''}</p>
                <p className="text-[var(--muted)]">{address.city}{address.state ? `, ${address.state}` : ''} {address.postalCode}</p>
              ...
              <div className="border-t border-[var(--border)] pt-4 space-y-1 text-sm">
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={() => setStep(1)} className="text-xs text-violet hover:underline">Edit</button>
                </div>
              ...
              <div className="border-t border-[var(--border)] pt-4 space-y-1 text-sm">
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={() => setStep(2)} className="text-xs text-violet hover:underline">Edit</button>
                </div>
              ...
              <div className="flex justify-between">
                <button type="button" onClick={() => setStep(2)} className="text-sm text-[var(--muted)] hover:text-[var(--text)]">
                  ← Back
                </button>
                <button
                  type="button"
        ...
        {cart && (
          <div className="glass rounded-3xl p-5 space-y-4 h-fit">
              ...
              {cart.items.map((item) => (
                <div key={`${item.product._id}-${item.variant?.value ?? ''}`} className="flex gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.variant && <p className="text-xs text-[var(--muted)]">{item.variant.value}</p>}
                    <p className="text-xs text-[var(--muted)]">Qty {item.qty}</p>
                  </div>
                  <p className="text-xs font-semibold shrink-0">${(item.product.price * item.qty).toFixed(2)}</p>
                </div>
              ))}
            </div>
            <div className="space-y-1.5 text-sm border-t border-[var(--border)] pt-3">
              ...
              <div className="flex justify-between text-[var(--muted)]">
                <span>Shipping</span>
                <span>{step >= 1 ? (shipping.price === 0 ? 'Free' : `$${shipping.price.toFixed(2)}`) : '—'}</span>
              </div>
